...
void lptoken::transfer(const name &from, const name &to, const asset &quantity, const string &memo) {
    ...
    action(permission_level{_self, "active"_n}, _self, "changelog"_n, data).send();
}
...
uint64_t lptoken::sub_balance(const name &owner, const asset &value) {
    ...
    if (owner != _self && owner != SWAP_CONTRACT) {
        ...
        action(permission_level{_self, "active"_n}, LPPOOL_CONTRACT, "lpchange"_n, data).send();
    }
...
void lptoken::changelog(symbol_code code, const name &from, const name &to, uint64_t amount, uint64_t from_balance, uint64_t to_balance) {
    require_auth(_self);
}
