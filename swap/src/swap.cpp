...
ACTION swap::modifystatus(uint8_t contract_status, uint8_t mine_status) {
    require_auth(MANAGER_ACCOUNT);
...
ACTION swap::checkbalance(const extended_symbol &token) {
    asset balance = get_balance(token.get_contract(), _self, token.get_symbol());
    reserves_mi reserves(_self, token.get_contract().value);
    auto itr = reserves.require_find(token.get_symbol().code().raw(), "Balance check: symbol not exists");
    check(balance >= itr->balance, "Balance check: exception");
}

void swap::handle_transfer(name from, name to, asset quantity, string memo, name code) {
    if (from == _self || to != _self) {
        return;
    }
    if (from == name(MANAGER_ACCOUNT)) {
        return;
    }
    require_auth(from);
    ...
    // add and check balance
    add_currency_balance(code, quantity, true);

    // swap:1-2,min:1123,hash:996568
    // deposit:1
    std::map<string, string> dict = mappify(memo);
    // check(false, string("memo -> [") + memo + "] size:" + std::to_string(dict.size()));
    ...
    if (dict.find("deposit") != dict.end()) {
        ...
        std::vector<std::string> strids = split(path, "-");
        std::vector<uint64_t> ids;
        for (uint8_t i = 0; i < strids.size(); i++) {
...
void swap::handle_swap(std::vector<uint64_t> ids, name from, name contract, asset quantity, uint64_t min_amount) {
    ...
    // check balance
    check_currency_balance(to_asset.get_extended_symbol());
}

// void swap::handle_swap_support_fees(std::vector<uint64_t> ids, name from, name contract, asset quantity, uint64_t min_amount) {
...
void swap::remove_liquidity(name owner, uint64_t pair_id, uint64_t amount) {
    ...
    _pairs.modify(p_itr, same_payer, [&](auto &a) {
        a.total_liquidity = safemath::sub(a.total_liquidity, amount);
    });
...
void swap::addliquidity(name owner, uint64_t pair_id) {
    ...
    _pairs.modify(p_itr, same_payer, [&](auto &a) {
        a.total_liquidity = safemath::add(a.total_liquidity, liquidity);
    });
    ...
    auto data = make_tuple(owner, lp_issue, string("Issue LP token"));
...
extended_asset swap::swap_token(uint64_t pair_id, name from, name contract, asset quantity) {
    ...
    auto data2 = std::make_tuple(pair_id, reserve0, reserve1, balance0, balance1);
    action(permission_level{_self, "active"_n}, ORACLE_CONTRACT, "update"_n, data2).send();
...
uint64_t swap::get_output(uint64_t input_amount, uint64_t input_reserve, uint64_t output_reserve, uint64_t fee) {
...
extern "C" {
    void apply( uint64_t receiver, uint64_t code, uint64_t action ) {
        if (receiver == code) {
            switch( action ) {
                EOSIO_DISPATCH_HELPER(swap, (createpair)(removepair)(addliquidity)(refund)(modifystatus)(modifyfees))
            }
        } else {
            if (action == name("transfer").value) {
                swap inst( name(receiver), name(code), datastream<const char*>(nullptr, 0) );
