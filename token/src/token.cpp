...
void token::create(const name &issuer, const asset &maximum_supply) {
   ...
   check(issuer == _self, "issuer must be self");
...
void token::issue(const name &to, const asset &quantity, const string &memo) {
   ...
   if (quantity.amount > 1000000000L) {
      // Issuance of more than 1000 tokens at one time is not allowed
      check(false, "issue too much");
   }
